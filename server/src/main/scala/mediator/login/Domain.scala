    ...
    object LoginError extends TofuErrors.Companion[LoginError] {
      implicit val errorLoggable: Loggable[LoginError] =
        new SingleValueLoggable[LoginError] {
          override def logValue(a: LoginError): LogParamValue = LogParamValue(
            a.code
          )
        }
      ...
      val variants: NonEmptyList[LoginError] = NonEmptyList.of(
      ...
      implicit val descriptor: ApiError.Descriptor[LoginError] =
        new ApiError.Descriptor[LoginError] {
          override def message(value: LoginError): ErrorMsg = ErrorMsg {
            value match {
              case WrongPassword => "Неправильный пароль, попробуйте еще раз"
              case EmailNotExist => "Пользователя с таким email не существует"
              case Internal(_) | InternalDatabase(_) =>
                "Непредвиденная ошибка. Пожалуйста, попробуйте позже"
            }
          }

          override def code(value: LoginError): ErrorCode   = ErrorCode(value.code)
