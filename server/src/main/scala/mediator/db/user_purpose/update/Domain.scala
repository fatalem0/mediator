    ...
    object UpdateError {
      ...
      object ForeignKey {
        case object UserIdNotFound
          extends ForeignKey("user_purposes_users_user_id_fkey")

        case object UserPurposeIdNotFound
          extends ForeignKey("user_purposes_users_user_purpose_id_fkey")
      }
      ...
      def fromDatabaseError(error: DatabaseError): UpdateError =
        error match {
          ...
          case DatabaseError.Sql(cause: BatchUpdateException)
              if check(cause, ForeignKey.UserPurposeIdNotFound) | check(
                cause,
